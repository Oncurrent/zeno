...
;               :path path
;               :value "XYZ"}]
;         crdt (apply-ops/apply-ops (u/sym-map ops schema sys-time-ms))
;         expected-v "XYZ"]
;     (doseq [ops (combo/permutations ops)]
;       (let [crdt (apply-ops/apply-ops (u/sym-map ops
;                                             schema
;                                             sys-time-ms))
...
;   (let [schema the-rec-schema
;         crdt-1 (apply-ops/apply-ops {:ops [{:add-id "a1"
;                                             :op-type :add-value
;                                             :path [:foo/a 1]
;                                             :schema schema
;                                             :value 1}]
;                                      :schema schema
;                                      :sys-time-ms (u/str->long "1640205282858")})
;         crdt-2 (apply-ops/apply-ops {:crdt crdt-1
;                                      :ops [{:add-id "a2"
;                                             :op-type :add-value
;                                             :path [:foo/a 1]
;                                             :schema schema
...
(comment (kaocha.repl/run #'test-thingy))
(deftest test-thingy
...
#_(deftest thingy
  (let [schema (l/array-schema l/string-schema)
        ...
        *next-id-num (atom 0)
        ...
        expected-v ["A" "X" "Y" "C"]]
    ;; Too many permutations to test them all, so we take a random sample
    (let [crdt1 (apply-ops/apply-ops (-> (u/sym-map schema sys-time-ms)
                                         (assoc :ops ops1)))
          crdt2 (apply-ops/apply-ops (-> (u/sym-map schema sys-time-ms)
                                         (assoc :ops ops2)))]
      (is (= crdt1 crdt2))
      (let [crdt1* (-> (into (sorted-map) crdt1)
                       (update :children into (sorted-map))
                       (update :add-id-to-edge into (sorted-map))
                       (update :current-edge-add-ids into (sorted-set))
                       (update :deleted-edge-add-ids into (sorted-set)))
            crdt2* (-> (into (sorted-map) crdt2)
                       (update :children into (sorted-map))
                       (update :add-id-to-edge into (sorted-map))
                       (update :current-edge-add-ids into (sorted-set))
                       (update :deleted-edge-add-ids into (sorted-set)))
            _ (log/info (u/pprint-str crdt1*))
            _ (log/info (u/pprint-str crdt2*))
            _ (log/info (:add-id-to-edge crdt1*))
            _ (log/info (:add-id-to-edge crdt2*))
            _ (log/info "getting 1")
            v1 (crdt/get-value (-> (u/sym-map make-id path schema)
                                   (assoc :crdt crdt1*)))
            _ (log/info "getting 2")
            v2 (crdt/get-value (-> (u/sym-map make-id path schema)
                                   (assoc :crdt crdt2*)))
            ]))))

#_(deftest test-array-crdt-conflict
  (let [schema (l/array-schema l/string-schema)
        sys-time-ms (u/str->long "1640205282858")
        *next-id-num (atom 0)
        make-id #(let [n (swap! *next-id-num inc)]
    ...
    ;; Too many permutations to test them all, so we take a random sample
    ; (doseq [ops (repeatedly 500 #(shuffle ops))]
    (let [crdt (apply-ops/apply-ops (u/sym-map ops schema sys-time-ms))
...
;     (doseq [ops (combo/permutations ops)]
;       (let [crdt (apply-ops/apply-ops apply-ops-arg)
;             get-value-arg (u/sym-map crdt path schema)
;             v (crdt/get-value get-value-arg)
;             _ (is (= ["A" "B"] v))
;             crdt* (apply-ops/apply-ops (assoc apply-ops-arg
;                                               :crdt crdt
;                                               :ops change-ops))
...
;         ops []
;         path []
;         crdt (apply-ops/apply-ops (u/sym-map ops
;                                              schema
;                                              sys-time-ms))
;         v (crdt/get-value (u/sym-map crdt path schema))
;         _ (is (= nil v))]))
