      ...
      (let [config {:branch "integration-test"
        ...
        (try
          (let [_ (is (= 0 (count-lines extra-info)))
                ...
                _ (is (= session-token (:session-token rs-ret)))
                _ (is (= created-actor-id (:actor-id rs-ret)))
                _ (is (= false (au/<? (mta/<resume-session! zc2 "an-invalid-token"))))
                _ (is (= true (au/<? (mta/<log-out! zc))))
...
(comment
  ...
  (def config {:branch "integration-test"
               :get-server-url (constantly "ws://localhost:8080/client")})

  (def zc (zc/zeno-client config))

  (def identifier (make-identifier))
  (def created-actor-id (au/<?? (mta/<create-actor! zc identifier)))
